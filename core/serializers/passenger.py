...
class PassengerListAndRetrieveSerializer(ModelSerializer):
    passenger_id = serializers.IntegerField(source='user.id', read_only=True)
    ...
    class Meta:
        model = Passenger
        fields = ['id', 'passenger_id', 'name', 'profile_picture', 'cep', 'is_approved', 'group_route']
...
class PassengerPatchSerializer(ModelSerializer):
